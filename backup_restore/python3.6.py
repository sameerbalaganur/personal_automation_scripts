...
server_name = "POCWDEVOPSPDB01\MSSQL2016DPRI"
...
'''.format(shared_folder=shared_folder, database=database, server_name=server_name, backup_file_name=backup_file_name, username=username, password=password)

        remote.run(ps_script)

    except:
        print("Error connecting to {hostname}".format(hostname=hostname))
        sys.exit(1)
